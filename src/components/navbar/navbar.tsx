...
export default component$<{active?: ActiveTab}>(({active}) => {
  ...
  return (
    <>
      <header 
        class={
          "sticky top-0 left-0 right-0 text-mauved-50 py-3 h-[60px] z-10" + (
            mobileNavOpen.value 
              ? " backdrop-blur-none md:backdrop-blur" 
              : " backdrop-blur"
          )
        }
      >
      ...
      {/* Nav links (mobile) */}
      {mobileNavOpen.value && (
        <div class="block md:hidden fixed inset-0 backdrop-blur-lg z-[5] pt-[60px]">
          <div class="max-w-7xl mx-auto">
            <nav class="" aria-label="Primary">
              <ul class="flex flex-col space-y-5 px-2 pt-4 text-mauved-50 text-6xl font-semibold">
                {navLinks.map((link, i) => (
